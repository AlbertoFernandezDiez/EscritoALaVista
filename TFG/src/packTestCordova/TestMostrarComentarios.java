...
public class TestMostrarComentarios {
	...
	public void test() {
		...
		try {
			peticion = new PeticionPost("http://127.0.0.1:8080/TFG/api/MostrarComentarios");
			JSONObject json = null;
			try {
				peticion.add("idC", String.valueOf(idCap));
				peticion.add("idO", String.valueOf(idObra));



				json = new JSONObject(peticion.getRespueta());
				assertTrue(json.getJSONArray("comentarios").length() > 0);
				assertEquals(idUs, json.getJSONArray("comentarios").getJSONObject(0).get("idAutor"));
