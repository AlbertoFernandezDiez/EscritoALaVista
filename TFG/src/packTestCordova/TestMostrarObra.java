...
public class TestMostrarObra {
	...
	public void testDoPostHttpServletRequestHttpServletResponse() {
		...
		try {
			peticion = new PeticionPost("http://127.0.0.1:8080/TFG/api/MostrarObra");
			JSONObject json = null;
			try {
				peticion.add("id", String.valueOf(idObra));


				json = new JSONObject(peticion.getRespueta());
				assertEquals(String.valueOf(aux), json.get("titulo"));
				assertEquals(idObra, json.get("id"));
